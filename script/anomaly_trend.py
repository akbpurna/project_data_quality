...
from functions import days, del_dup_t
import warnings
...
query = """
    ...
    into temp location_t_{table_destination}
    from data_master.v_data_quality_{table_destination}
    where "level" = 'site' and 
    length(location) <> 6;

    select kpi
    into temp kpi_t_{table_destination}
    ...
    with data_cek as (
        ...
        where   date = '{date}'
                and kpi in (select kpi from kpi_t_{table_destination})
                and "location" not in (select location from location_t_{table_destination})
